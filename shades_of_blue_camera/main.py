...
cam.deinit()
pwdn.value = True  # camera off until needed
print(f"camera off ({cam_width}x{cam_height})")

# --- Bitmap setup ---
bitmap = displayio.Bitmap(cam_width, cam_height, 65535)
...
while True:
    try:
        ...
        if now - last_send >= SEND_INTERVAL:
            print(f"[{now:.1f}] Camera on, capturing...")
            pwdn.value = False  # LOW = power on
            cam = init_camera()
            cam.capture(bitmap)

            t0 = time.monotonic_ns()
            r, g, b = compute_average_color(bitmap, cam_width, cam_height, SAMPLE_STEP)
            t1 = time.monotonic_ns()
            elapsed_ms = (t1 - t0) // 1_000_000
            print(f"[{time.monotonic():.1f}] avg color: R={r} G={g} B={b} ({elapsed_ms}ms)")

            # Power down and deinit before network op
