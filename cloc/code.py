...
# Create list of valid combinations (excluding numbers) and shuffle once
valid_combinations = [c for c in range(128) if c not in DO_NOT_DISPLAY]

# Shuffle using Fisher-Yates algorithm (CircuitPython doesn't have random.shuffle)
for i in range(len(valid_combinations) - 1, 0, -1):
    j = random.randint(0, i)
    valid_combinations[i], valid_combinations[j] = valid_combinations[j], valid_combinations[i]

# Cycle through all valid combinations in random order
while True:
